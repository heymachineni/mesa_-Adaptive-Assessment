...
**Hosting: see `DEPLOY.md`.** Short version — this needs a persistent process
with a disk, so Railway, Render or Fly.io work and Vercel/Netlify cannot.
Config files for all three are in the repo, and the database seeds itself on
first boot. Set `DB_DIR=/data` and a real `ADMIN_KEY`.
